...
public class ListaIntEnlazada {
    ...
    /**
     * Clase NodoInt: representa un nodo de una secuencia enlazada que tiene un
     * dato de tipo int y un enlace al nodo siguiente.
     *
     * @author Jose
     * @version 2025
     */
    ...
    public boolean existeValor(int valor) {
        ...
        while (aux != null) {
            if (aux.dato == valor) {
                return true; //Esto es que el valor ya existe
            }
            aux = aux.siguiente;
        }
        return false; //Esto es que el valor ya no existe
    }

    public void insertarSinRepetidos(int valor, boolean delante) {
        if (existeValor(valor)) {
            return; // Si ya tiene valor no hay sentido de hacerlo
        }
        
        NodoInt nNodo = new NodoInt(valor);
        ...
        talla++; //lo vamos haciendo mas grande
    }
}
